...
/**
 * Universal Form Request for updating OwnershipType
...
class UpdateOwnershipTypeRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        return $this->user()?->can('update', $this->route(strtolower('OwnershipType'))) ?? false;
    }
    ...
    {
        $id = $this->route(strtolower('OwnershipType'))?->id ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique(strtolower('OwnershipTypes'))->ignore($id)],
    ...
    {
        logger()->warning('Update validation failed for UpdateOwnershipTypeRequest', [
