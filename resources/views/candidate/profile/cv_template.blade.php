        ...
        <div class="mt-3 flex flex-column">
            @isset($user->candidate->full_location)
                <div class="mb-1 flex items-center">
                    ...
                    <span class="text-gray-500 fs-3">{{ $user->candidate->full_location}}</span>
                </div>
            @endisset
            <div class="mb-1 flex items-center">
                <img class="me-3" src="{{ asset('assets/img/envelope.png') }}">
                <span class="text-gray-500 fs-3">{{ $user->email}}</span>
            </div>
            <div class="flex items-center">
                <img class="me-3" src="{{ asset('assets/img/phone.png') }}">
                <span class="text-gray-500 fs-3">{{ $user->phone}}</span>
            </div>
        </div>
    </div>
    <div class="flex-1 -md-12 mb-8 ps-7">
        @if($user->candidateSkill->count())
            ...
            @foreach($user->candidateSkill as $skill)
                <p class="text-gray-500 fs-3 custom-bullet">{{ $skill->name }}</p>
            @endforeach
        @endif
    </div>
    <div class="flex-1 -md-12 mb-8 ps-7">
        ...
        <div class="flex flex-wrap">
            @foreach($candidateEducations as $candidateEducation)
                <div class="flex-1 -6 mt-5">
                    <h5 class="fs-3 text-primary-600">{{ $candidateEducation->degreeLevel->name }}</h5>
                    <h6 class="text-gray-500 fs-5">{{ $candidateEducation->degree_title }}</h6>
                    <span
                            class="text-gray-500 fs-4">{{ $candidateEducation->year }} | {{ $candidateEducation->country }}</span>
    ...
    <div class="flex-1 -md-12 mb-8 ps-7">
        ...
        <div class="flex flex-wrap">
            @foreach($candidateExperiences as $candidateExperience)
                <div class="flex-1 -6 mt-5">
                    <h5 class="fs-3 text-primary-600">{{ $candidateExperience->company }}</h5>
                    <h6 class="fs-5">{{ $candidateExperience->country }}</h6>
                    <span class="text-gray-500 fs-4">{{ \Carbon\Carbon::parse($candidateExperience->start_date)->translatedFormat('jS M, Y')}} - </span>
                    @if($candidateExperience->currently_working)
                        <span class="text-gray-500 fs-4">{{ __('messages.candidate_profile.present') }}</span>
                    @else
                        <span
                            class="text-gray-500 fs-4"> {{\Carbon\Carbon::parse($candidateExperience->end_date)->translatedFormat('jS M, Y')}} </span>
                    @endif
                </div>
            @endforeach
        </div>
