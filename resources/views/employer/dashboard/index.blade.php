...
@section('content')
    <div class="flex flex-wrap">
        <div class="flex-1 -xl-4 flex-1 sm-6 widget">
            <a href="{{ route('employer.jobs.index') }}" class="text-decoration-none">
        ...
        <div class="flex-1 -xl-4 flex-1 sm-6 widget">
            <a href="{{ route('employer.jobs.index') }}" class="text-decoration-none">
            <div class="bg-indigo-600 -600 shadow-md rounded -10 p-xxl-10 px-5 py-10 flex items-center justify-between my-sm-3 my-2">
        ...
        <div class="flex-1 -xl-4 flex-1 sm-6 widget">
            <div class="bg-gray-800 shadow-md rounded -10 p-xxl-10 px-5 py-10 flex items-center justify-between my-sm-3 my-2">
                <div
                    ...
                    <i class="fas fa-file fa-4x fs-1-xl {{ getLoggedInUser()->theme_mode ? ' text-gray-500 ' : ' text-white ' }}"></i>
                </div>
                <div class="text-end {{ getLoggedInUser()->theme_mode ? ' text-gray-500 ' : ' text-white ' }}">
    ...
    <div class="bg-white rounded-lg shadow-md border border-gray-300 border border-gray-300 -gray-200 bg-white shadow rounded -lg overflow-hidden xl-stretch mb-xl-8">
        <!--begin::Header-->
        <div class="bg-white shadow rounded -lg overflow-hidden header border border-gray-300 -0 pt-5">
            <h3 class="bg-white shadow rounded -lg overflow-hidden title items-start flex- flex-1">
                                    <span
            ...
            <div class="lg:w-8/12 px-2 md:w-8/12 flex-1 sm-12">
                <div class="flex flex-wrap justify-end">
                    <div class="lg:w-4/12 px-2 md:w-4/12 flex-1 -xl-3 flex-1 sm-4 mt-3 mt-md-0">
                        <div class="bg-white shadow rounded -lg overflow-hidden header-action w-full">
                            {{ Form::select('jobs', $jobStatus, null, ['id' => 'jobStatus', 'class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm status-filter', 'placeholder' => __('messages.flash.select_job')]) }}
                        </div>
                    </div>
                    <div class="lg:w-4/12 px-2 md:w-4/12 flex-1 -xl-3 flex-1 sm-4 mt-3 mt-md-0">
                        <div class="bg-white shadow rounded -lg overflow-hidden header-action w-full">
                            {{ Form::select('gender', $gender, null, ['id' => 'gender', 'class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm status-filter', 'placeholder' => __('messages.company.select_gender')]) }}
                        </div>
                    </div>
                    <div class="lg:w-4/12 px-2 md:w-4/12 flex-1 -xl-4 flex-1 sm-4 mt-0">
                        <div id="timeRange" class="time_range time_range_width w-30 border border border-gray-300 -gray-300 rounded -2 p-3">
    ...
    <div class="flex flex-wrap">
        ...
        <div class="flex-1 xl-6 ps-0">
            ...
            <div class="mb-xl-8">
                <!--begin::Header-->
                <div class="flex justify-between border border-gray-300 -0 pt-5">
                    <h3 class="items-start flex- flex-1">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 w-full divide-y divide-gray-200 striped align-middle gs-0 gy-5">
                            ...
                            @if(count($recentJobs) > 0)
                                @foreach($recentJobs as $recentJob)
                                    <tr>
                                        <td class="ps-3">
                                            <a href="{{ route('front.',$recentJob->job_id) }}"
                                        ...
                                        <td class="text-center">
                                            <div
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded -full text-xs font-medium w-auto bg-{{ \App\Models\Job::STATUS_COLOR[$recentJob->status] }}">
                                                <span
