...
checkResult("AuthenticateMiddleware exists", 
    file_exists('app/Http/Middleware/AuthenticateMiddleware.php'), 
    $security_total, $security_passed);

checkResult("SecurityHeadersMiddleware exists", 
    file_exists('app/Http/Middleware/SecurityHeadersMiddleware.php'), 
    $security_total, $security_passed);

checkResult("Sanctum config exists", 
    file_exists('config/sanctum.php'), 
    $security_total, $security_passed);

if (file_exists('.env')) {
    $env_content = file_get_contents('.env');
    checkResult("APP_DEBUG is false", 
        strpos($env_content, 'APP_DEBUG=false') !== false, 
        $security_total, $security_passed);
    
    checkResult("Security headers enabled", 
        strpos($env_content, 'SECURITY_HEADERS_ENABLED=true') !== false, 
        $security_total, $security_passed);
}
...
if (file_exists('composer.json')) {
    ...
    checkResult("Laravel Framework updated", 
        isset($composer['require']['laravel/framework']) && 
        strpos($composer['require']['laravel/framework'], '^12.17') !== false, 
        $deps_total, $deps_passed);
    
    checkResult("Laravel Sanctum updated", 
        isset($composer['require']['laravel/sanctum']) && 
        strpos($composer['require']['laravel/sanctum'], '^4.0') !== false, 
        $deps_total, $deps_passed);
}
...
if (file_exists('package.json')) {
    ...
    checkResult("Vite updated", 
        isset($package['devDependencies']['vite']) && 
        strpos($package['devDependencies']['vite'], '^5.4') !== false, 
        $deps_total, $deps_passed);
    
    checkResult("TailwindCSS updated", 
        isset($package['devDependencies']['tailwindcss']) && 
        strpos($package['devDependencies']['tailwindcss'], '^3.4') !== false, 
        $deps_total, $deps_passed);
    
    checkResult("Laravel Vite Plugin compatible", 
        isset($package['devDependencies']['laravel-vite-plugin']) && 
        strpos($package['devDependencies']['laravel-vite-plugin'], '^1.0') !== false, 
        $deps_total, $deps_passed);
}
...
checkResult("App.vue exists", 
    file_exists('resources/js/App.vue'), 
    $vue_total, $vue_passed);

checkResult("NavBar component exists", 
    file_exists('resources/js/components/layout/NavBar.vue'), 
    $vue_total, $vue_passed);

checkResult("Footer component exists", 
    file_exists('resources/js/components/layout/Footer.vue'), 
    $vue_total, $vue_passed);

checkResult("Main.ts exists", 
    file_exists('resources/js/main.ts'), 
    $vue_total, $vue_passed);

checkResult("Home page exists", 
    file_exists('resources/js/pages/Home.vue'), 
    $vue_total, $vue_passed);

checkResult("UI components directory exists", 
    is_dir('resources/js/components/ui'), 
    $vue_total, $vue_passed);

checkResult("Forms components directory exists", 
    is_dir('resources/js/components/forms'), 
    $vue_total, $vue_passed);

checkResult("Stores directory exists", 
...
if ($migrate_output) {
    ...
    foreach ($migrate_output as $line) {
        if (strpos($line, 'create_permissions_table') !== false && strpos($line, 'Ran') !== false) {
            $has_permissions = true;
        }
        if (strpos($line, 'create_roles_table') !== false && strpos($line, 'Ran') !== false) {
            $has_roles = true;
        }
    }
...
checkResult("Build manifest exists", 
    file_exists('public/build/manifest.json'), 
    $build_total, $build_passed);

if (file_exists('public/build/manifest.json')) {
    $manifest = json_decode(file_get_contents('public/build/manifest.json'), true);
    checkResult("Assets compiled", 
        count($manifest) > 0, 
        $build_total, $build_passed);
    
    checkResult("CSS assets exist", 
        count(array_filter($manifest, function($asset) { 
            return isset($asset['file']) && strpos($asset['file'], '.css') !== false; 
        })) > 0, 
        $build_total, $build_passed);
    
    checkResult("JS assets exist", 
        count(array_filter($manifest, function($asset) { 
            return isset($asset['file']) && strpos($asset['file'], '.js') !== false; 
        })) > 0, 
        $build_total, $build_passed);
}
...
checkResult("Laravel command working", 
    $version_exit === 0 && count($version_output) > 0, 
    $app_total, $app_passed);
...
echo "Production Mode: " . (strpos(file_get_contents('.env'), 'APP_DEBUG=false') !== false ? "✅ Enabled" : "❌ Debug Mode") . "\n";
