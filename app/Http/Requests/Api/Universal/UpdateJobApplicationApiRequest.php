...
/**
 * Universal API Request for updating JobApplication
 * Implements Laravel 12 API best practices with Universal MCP patterns.
 */
class UpdateJobApplicationApiRequest extends FormRequest
{
    /**
    ...
    {
        $canUpdate = $this->user()?->tokenCan(strtolower('JobApplication').':update') ?? false;
        $resource = $this->route(strtolower('JobApplication'));
    ...
    {
        $id = $this->route(strtolower('JobApplication'))?->id ?? $this->route('id');

        return [
            'name' => ['sometimes', 'string', 'max:255', Rule::unique(strtolower('JobApplications'))->ignore($id)],
