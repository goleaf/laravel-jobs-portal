...
class PostCategory extends Model
{
    ...
    public function activePosts(): BelongsToMany
    {
        return $this->posts()->where('posts.is_active', true);
    }

    public function featuredPosts(): BelongsToMany
    {
        return $this->posts()->where('posts.is_featured', true);
    }

    public function recentPosts(): BelongsToMany
    {
        return $this->posts()->where('posts.created_at', '>=', now()->subDays(30));
    }
    ...
    public function scopeWithFeaturedPosts($query)
    {
        return $query->whereHas('posts', function ($q) {
    ...
    {
        return $query->withCount('posts')->having('posts_count', '=', 0);
    }
    ...
    public function getFeaturedPostsCountAttribute(): int
    {
        return $this->featuredPosts()->count();
    }

    public function getRecentPostsCountAttribute(): int
    {
    ...
    public function hasActivePosts(): bool
    {
        return $this->activePosts()->count() > 0;
    }

    public function hasFeaturedPosts(): bool
    {
        return $this->featuredPosts()->count() > 0;
    }
    ...
    public function getStatsAttribute(): array
    {
        return [
            ...
            'active_posts' => $this->activePosts()->count(),
            'featured_posts' => $this->featuredPosts()->count(),
    ...
    public function clearCaches(): void
    {
        ...
        for ($i = 5; $i <= 20; $i += 5) {
            $cacheKeys[] = "post_categories.popular.{$i}";
        }
