    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        <div class="bg-white shadow rounded-lg overflow-hidden body">
            ...
            <div class="mt-5">
                ...
                <div class="flex flex-wrap mb-5">
                    ...
                    <div class="col-xl-6 md:w-6/12 flex-1 sm-6 mb-5">
                        ...
                        {{ Form::text('first_name', $user->first_name, ['class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm','required','placeholder'=> __('messages.candidate.first_name')]) }}
                    </div>
                    <div class="col-xl-6 md:w-6/12 flex-1 sm-6 mb-5">
                        {{ Form::label('last_name', __('messages.candidate.last_name').':', ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
                        <span class="required"></span>
                        {{ Form::text('last_name', $user->last_name,['class' =>'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm ','required','placeholder'=> __('messages.candidate.last_name')]) }}
                    </div>
                    <div class="col-xl-6 md:w-6/12 flex-1 sm-6  mb-5">
                        {{ Form::label('email', __('messages.company.email').':', ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
                        <span class="required"></span>
                        {{ Form::email('email', isset($user)?$user->email:null, ['class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm ', 'required','placeholder'=> __('messages.company.email')]) }}
                    </div>
                    <div class="col-xl-6 md:w-6/12 flex-1 sm-6 mb-5">
                        {{ Form::label('father_name', __('messages.candidate.father_name').':', ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
                        <span class="required"></span>
                        {{ Form::text('father_name', $user->$candidate->father_name, ['class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm','required','placeholder'=> __('messages.candidate.father_name')]) }}
                    </div>
                    <div class="col-xl-6 md:w-6/12 flex-1 sm-6 mb-5">
                ...
                <br>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.common.female') }}</label>
                {{ Form::radio('gender', '1', isset($user->gender) ? $user->gender == 1 : true, ['class' => 'form-check-input','id'=>'female']) }}
                    ...
                    <div class="col-xl-6 md:w-6/12 flex-1 sm-12 mb-5">
                        {{ Form::label('national_id_card', __('messages.candidate.national_id_card').':', ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
                        {{ Form::text('national_id_card', isset($user->$candidate->national_id_card) ? $user->$candidate->national_id_card : null, ['class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm ', 'placeholder' => __('messages.candidate.national_id_card') ]) }}
                    </div>
                    <div class="col-xl-6 md:w-6/12 flex-1 sm-12 mb-5">
            ...
            {{ Form::select('city_id',(isset($cities) && $cities!=null?$cities:[]), null,['class' => 'form-select ','id'=>'cityId','placeholder' => __('messages.company.select_city')]) }}
                    ...
                    <div class="flex-1 sm-6 mb-5 mobile-itel-width">
                ...
                {{ Form::hidden('region_code',null,['id'=>'prefix_code']) }}
                        <span id="valid-msg" class="text-green-600 block fw-400 fs-small mt-2 hidden">{{ __('messages.phone.valid_number') }}</span>
                        <span id="error-msg" class="text-red-600 block fw-400 fs-small mt-2 hidden"></span>
                    </div>
                ...
                <br>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.candidate.not_immediate_available') }}</label>
                ...
                <div class="flex">
                    ...
                    <div class="flex-1 sm-6 mb-5">
                        <div class="mb-3" io-image-input="true">
                            <label for="exampleInputImage" class="block text-sm font-medium text-gray-700 mb-1"> {{ __('messages.candidate.profile') }}:</label>
            ...
            {{ Form::textarea('address', isset($user->$candidate->address) ? $user->$candidate->address : null, ['class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm','rows'=>'5','placeholder'=>__('messages.candidate.address')]) }}
        </div>

        <!-- Submit Field -->
        <div class="flex justify-end">
            {{ Form::submit(__('messages.common.save'), ['class' => 'rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none transition-colors me-3 btnSave']) }}
            {{ --                <a href=""-- }}
