...
/**
 * Universal Form Request for updating Health
...
class UpdateHealthRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        return $this->user()?->can('update', $this->route(strtolower('Health'))) ?? false;
    }
    ...
    {
        $id = $this->route(strtolower('Health'))?->id ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique(strtolower('Healths'))->ignore($id)],
    ...
    {
        logger()->warning('Update validation failed for UpdateHealthRequest', [
