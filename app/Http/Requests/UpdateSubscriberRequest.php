...
/**
 * Universal Form Request for updating Subscriber
...
class UpdateSubscriberRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        return $this->user()?->can('update', $this->route(strtolower('Subscriber'))) ?? false;
    }
    ...
    {
        $id = $this->route(strtolower('Subscriber'))?->id ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique(strtolower('Subscribers'))->ignore($id)],
    ...
    {
        logger()->warning('Update validation failed for UpdateSubscriberRequest', [
