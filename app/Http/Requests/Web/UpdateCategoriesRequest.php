...
/**
 * Context7 Form Request for updating Categories
...
class UpdateCategoriesRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        return $this->user()?->can('update', $this->route(strtolower('Categories'))) ?? false;
    }
    ...
    {
        $id = $this->route(strtolower('Categories'))?->id ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique(strtolower('Categoriess'))->ignore($id)],
    ...
    {
        logger()->warning('Update validation failed for UpdateCategoriesRequest', [
