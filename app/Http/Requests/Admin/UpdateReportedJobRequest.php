...
/**
 * Universal Form Request for updating ReportedJob
 * Implements Laravel 12 best practices with Universal MCP patterns
 */
class UpdateReportedJobRequest extends FormRequest
{
    /**
    ...
    public function authorize(): bool
    {
        ...
        return $this->user()?->can('update', $this->route(strtolower('ReportedJob'))) ?? false;
    }
    ...
    {
        $id = $this->route(strtolower('ReportedJob'))?->id ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique(strtolower('ReportedJobs'))->ignore($id)],
    ...
    {
        logger()->warning('Update validation failed for UpdateReportedJobRequest', [
