...
/**
 * Universal Form Request for updating Sitemap
...
class UpdateSitemapRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        return $this->user()?->can('update', $this->route(strtolower('Sitemap'))) ?? false;
    }
    ...
    {
        $id = $this->route(strtolower('Sitemap'))?->id ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique(strtolower('Sitemaps'))->ignore($id)],
    ...
    {
        logger()->warning('Update validation failed for UpdateSitemapRequest', [
