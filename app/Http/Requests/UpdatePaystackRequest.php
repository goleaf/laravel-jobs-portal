...
class UpdatePaystackRequest extends FormRequest
{
    ...
    {
        $id = $this->route(strtolower('Paystack'))?->id ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique(strtolower('Paystacks'))->ignore($id)],
    ...
    {
        logger()->warning('Update validation failed for UpdatePaystackRequest', [
