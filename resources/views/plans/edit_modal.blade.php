        ...
        <div class="bg-white rounded-lg shadow-xl max-w-lg w-full">
            ...
            <div class="px-6 py-4">
                ...
                <div class="mb-5">
                    ...
                    {{ Form::text('name', null, ['id'=>'editName','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm','required', 'placeholder' => __('messages.inquiry.name')]) }}
                </div>
                <div class="mb-5">
                    {{ Form::label('allowed_jobs', __('messages.plan.allowed_jobs').(':'), ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
                    <span class="required"></span>
                    {{ Form::text('allowed_jobs', null, ['id'=>'editAllowedJobs','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm allowed-jobs','required', 'min' => '1', 'max' => '99999', 'placeholder' => __('messages.plan.allowed_jobs'), 'onkeyup' => 'if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,"")']) }}
                </div>
                <div class="mb-5">
                ...
                <div class="mb-5">
                    ...
                    <span class="required"></span>
                    {{ Form::text('amount', null, ['id'=>'editAmount','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm amount','required', 'min' => '1', 'max' => '99999','readonly','placeholder' => __('messages.plan.amount')]) }}
                </div>
            </div>
