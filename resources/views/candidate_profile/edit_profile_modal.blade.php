    ...
    <div class="flex items-center justify-center min-h-screen px-4 modal-lg">
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-lg w-full">
            ...
            <div class="px-6 py-4">
                ...
                <div class="flex flex-wrap">
                    <div class="form-group flex-1 sm-6 mb-5">
                        ...
                        {{ Form::text('first_name', null, ['id'=>'firstName','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm','required', 'placeholder' => __('messages.candidate.first_name')]) }}
                    </div>
                    <div class="form-group flex-1 sm-6 mb-5">
                        {{ Form::label('last_name',__('messages.candidate.last_name').(':'),['class' => 'block text-sm font-medium text-gray-700 mb-1 required']) }}
                        {{ Form::text('last_name', null, ['id'=>'lastName','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm', 'placeholder' => __('messages.candidate.last_name')]) }}
                    </div>
                </div>
                <div class="flex flex-wrap">
                    <div class="form-group flex-1 sm-6 mb-5">
                        {{ Form::label('email', __('messages.candidate.email').(':'),['class' => 'required block text-sm font-medium text-gray-700 mb-1']) }}
                        {{ Form::text('email', null, ['id'=>'editEmail','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm','required', 'placeholder' => __('messages.candidate.email')]) }}
                    </div>
                    <div class="form-group flex-1 sm-6 mb-5">
                    <div io-image-input="true">
                    {{ Form::label('', __('messages.candidate.profile').':', ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
            ...
            <div class="px-6 py-4 border-t border-gray-200 flex justify-end space-x-2 pt-0">
                {{ Form::button(__('messages.common.save'), ['type'=>'submit','class' => 'rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none transition-colors m-0','id'=>'btnPrEditSave','data-loading-text'=>"<span class="spinner-border spinner-border-sm"></span>".__('messages.common.process')]) }}
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-lg w-full">
            ...
            @formOpen(['id'=>'changeCandidateLanguageForm'])
                ...
                <div class="px-6 py-4 border-t border-gray-200 flex justify-end space-x-2 pt-0">
                    {{ Form::button(__('messages.common.save'), ['type' => 'submit','class' => 'rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none transition-colors m-0', 'id'=>'btnLanguageChange']) }}
                    <button type="button" class="inline-flex items-center px-4 py-2 border border-gray-300 border-transparent text-sm font-medium rounded-md transition duration-150 ease-in-out px-4 py-2 rounded font-medium transition-colors secondary my-0 ms-5 me-0"
