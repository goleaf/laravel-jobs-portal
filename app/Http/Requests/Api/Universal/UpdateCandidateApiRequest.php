...
/**
 * Universal API Request for updating Candidate
 * Implements Laravel 12 API best practices with Universal MCP patterns.
 */
class UpdateCandidateApiRequest extends FormRequest
{
    /**
    ...
    {
        $canUpdate = $this->user()?->tokenCan(strtolower('Candidate').':update') ?? false;
    ...
    {
        $id = $this->route(strtolower('Candidate'))?->id ?? $this->route('id');

        return [
            'name' => ['sometimes', 'string', 'max:255', Rule::unique(strtolower('Candidates'))->ignore($id)],
