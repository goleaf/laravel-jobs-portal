...
/**
 * Universal API Request for updating Company
...
class UpdateCompanyApiRequest extends FormRequest
{
    ...
    {
        $canUpdate = $this->user()?->tokenCan(strtolower('Company') . ':update') ?? false;
        $resource = $this->route(strtolower('Company'));
    ...
    {
        $id = $this->route(strtolower('Company'))?->id ?? $this->route('id');

        return [
            'name' => ['sometimes', 'string', 'max:255', Rule::unique(strtolower('Companys'))->ignore($id)],
