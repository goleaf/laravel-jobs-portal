...
/**
 * Universal API Request for updating User
...
class UpdateUserApiRequest extends FormRequest
{
    ...
    {
        $canUpdate = $this->user()?->tokenCan(strtolower('User') . ':update') ?? false;
        $resource = $this->route(strtolower('User'));
    ...
    {
        $id = $this->route(strtolower('User'))?->id ?? $this->route('id');

        return [
            'name' => ['sometimes', 'string', 'max:255', Rule::unique(strtolower('Users'))->ignore($id)],
