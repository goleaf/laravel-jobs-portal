      ...
      <div class="flex justify-between items-center h-16">
        ...
        <nav class="hidden lg:flex items-center space-x-8">
          <router-link 
            ...
            :key="item.name"
            :to="item.href"
            class="text-neutral-600 hover:text-primary-600 font-medium transition-colors relative group"
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="hidden md:block relative">
            ...
            <input
              ...
              class="pl-10 pr-4 py-2 w-64 text-sm border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all"
              @keyup.enter="handleSearch"
            />
          </div>

          <!-- Search Button (Mobile) -->
          <button class="md:hidden p-2 text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 rounded-md transition-colors">
            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </button>
          ...
          <div class="relative">
            <button 
              ...
              class="p-2 text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 rounded-md transition-colors relative"
            >
              <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div 
              ...
              class="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-neutral-200 py-2 z-50"
            >
              <div class="px-4 py-2 border-b border-neutral-200">
                <h3 class="text-sm font-semibold text-neutral-900">Notifications</h3>
              </div>
              <div class="max-h-64 overflow-y-auto">
                <div v-if="notifications.length === 0" class="px-4 py-8 text-center text-neutral-500">
                  No notifications
                </div>
                <div v-else>
                  <div 
                    ...
                    :key="notification.id"
                    class="px-4 py-3 hover:bg-neutral-50 cursor-pointer border-b border-neutral-100"
                    @click="handleNotificationClick(notification)"
                  >
                    <p class="text-sm text-neutral-900">{{ notification.title }}</p>
                    <p class="text-xs text-neutral-500 mt-1">{{ notification.time }}</p>
                  </div>
                </div>
              </div>
              <div class="px-4 py-2 border-t border-neutral-200">
                <router-link to="/notifications" class="text-sm text-primary-600 hover:text-primary-700">
          ...
          <div class="relative">
            ...
            >
              ...
              <div class="border-t border-neutral-200 py-1">
                <button 
                  @click="handleLogout"
                  class="block w-full text-left px-4 py-2 text-sm text-error-600 hover:bg-neutral-100 transition-colors"
                >
                  Sign out
                </button>
              </div>
