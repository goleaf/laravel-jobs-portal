...
/**
 * Context7 Test Data Generator
 * Creates sample data for Context7 API testing
 */
...
echo "🌱 CONTEXT7 TEST DATA GENERATOR\n";
echo "=" . str_repeat("=", 40) . "\n\n";
...
try {
    ...
    for ($i = 1; $i <= 3; $i++) {
        $user = User::create([
            'name' => "Context7 User $i",
            'email' => "user$i@context7.dev",
    ...
    foreach ($users as $index => $user) {
        $company = Company::create([
            'name' => "Context7 Company " . ($index + 1),
            'email' => "company" . ($index + 1) . "@context7.dev",
            'phone' => "555-010" . ($index + 1),
            'website' => "https://company" . ($index + 1) . ".context7.dev",
            'description' => "A leading company in the Context7 industry, focusing on innovation and excellence.",
            'location' => "Context7 City, State",
            'user_id' => $user->id,
            'is_active' => true,
        ]);
        $companies[] = $company;
    ...
    echo "\n🎉 CONTEXT7 TEST DATA CREATED SUCCESSFULLY!\n";
    echo "✅ Ready for Context7 API testing\n";
