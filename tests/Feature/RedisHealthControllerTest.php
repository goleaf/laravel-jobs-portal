...
class RedisHealthControllerTest extends TestCase
{
    ...
    public function test_index_displays_correctly(): void
    {
        $response = $this->actingAs($this->user)
            ->get(route('redishealth.index'));

        $response->assertStatus(200);
        $response->assertViewIs('redishealth.index');
    }
    ...
    {
        $response = $this->get(route('redishealth.index'));
    ...
    public function test_store_creates_new_record(): void
    {
        ...
        $response = $this->actingAs($this->user)
            ->post(route('redishealth.store'), $data);

        // Adjust based on actual controller behavior
        $response->assertRedirect();
        // $this->assertDatabaseHas('redishealths', ['name' => $data['name']]);
    }
    ...
    public function test_update_modifies_record(): void
    {
        ...
        $response = $this->actingAs($this->user)
            ->put(route('redishealth.update', 1), $newData);

        $response->assertRedirect();
        // $this->assertDatabaseHas('redishealths', ['id' => 1, 'name' => 'Updated Name']);
    }
    ...
    {
        $response = $this->actingAs($this->user)
            ->delete(route('redishealth.destroy', 1));

        $response->assertRedirect();
        // $this->assertSoftDeleted('redishealths', ['id' => 1]);
    }
    ...
    {
        $response = $this->get(route('redishealth.create'));
    ...
    public function test_handles_invalid_input_gracefully(): void
    {
        ...
        $response = $this->actingAs($this->user)
            ->post(route('redishealth.store'), $invalidData);
    ...
    public function test_search_functionality(): void
    {
        ...
        $response = $this->actingAs($this->user)
            ->get(route('redishealth.index', ['search' => $searchTerm]));
    ...
    {
        $response = $this->actingAs($this->user)
            ->get(route('redishealth.index', ['page' => 2]));
    ...
    public function test_csrf_protection_is_enforced(): void
    {
        ...
        $response = $this->post(route('redishealth.store'), $data);
    ...
    public function test_rate_limiting_prevents_abuse(): void
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            $this->actingAs($this->user)
                ->post(route('redishealth.store'), ['name' => 'Test ' . $i]);
        }
    ...
    public function test_handles_server_errors_gracefully(): void
    {
        ...
        $response = $this->actingAs($this->user)
            ->get(route('redishealth.show', 'invalid-id'));
