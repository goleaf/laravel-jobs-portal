...
class AdminOperationsRequest extends FormRequest
{
    ...
    {
        if ($this->has('user_ids')) return 'user_management';
        if ($this->has('system_maintenance_mode')) return 'system_settings';
        if ($this->has('audit_logging_enabled')) return 'security_audit';
        if ($this->has('scheduled_maintenance')) return 'maintenance';
