            ...
            <div class="flex flex-wrap">
                @foreach($featuredCategories as $featuredCategory)
                    <div class="lg:w-3/12 px-2 flex-1 md-6 px-xl-3 mb-40">
                        <div class="bg-white shadow rounded -lg overflow-hidden py-30 px-30 h-full border border border-gray-300 -gray-300 -0">
                            <div class="icon-box flex items-center justify-center mb-20">
                                <div class="icon-wrap bg-gray flex items-center justify-center br-5">
                                    @if($loop->index == 0)
                                        <x-icons.briefcase class="w-10 h-10 text-indigo-600-600" />
                                    @elseif($loop->index == 1)
                                        <x-icons.graduation-cap class="w-10 h-10 text-indigo-600-600" />
                                    @elseif($loop->index == 2)
                                        <x-icons.factory class="w-10 h-10 text-indigo-600-600" />
                                    @elseif($loop->index == 3)
                                        <x-icons.badge class="w-10 h-10 text-indigo-600-600" />
                                    @elseif($loop->index == 4)
                                        <x-icons.money class="w-10 h-10 text-indigo-600-600" />
                                    @else
                                        <x-icons.puzzle class="w-10 h-10 text-indigo-600-600" />
                                    @endif
                                </div>
                            </div>
                            <div class="bg-white shadow rounded -lg overflow-hidden body p-0 text-center">
                                <a href="{{ route('front.', array('categories' => $featuredCategory->id)) }}"
