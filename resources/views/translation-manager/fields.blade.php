<div class="flex flex-wrap flex-1 -12">
    ...
    <div class="flex-1 sm-3 mb-5 flex justify-end offset-3">
        <a class="rounded-md transition">{{ __('messages.common.add') }}</a>
    ...
    @foreach($languages as $key => $value)
        @if(!is_array($value))
            <div class="flex-1 sm-2 mb-5">
                ...
                {{ Form::text($key, $value, ['class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm','text-red-500','placeholder' => str_replace('_',' ',ucfirst($key))]) }}
            </div>
        @else
