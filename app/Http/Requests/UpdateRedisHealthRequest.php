...
/**
 * Universal Form Request for updating RedisHealth
 * Implements Laravel 12 best practices with Universal MCP patterns.
 */
class UpdateRedisHealthRequest extends FormRequest
{
    /**
    ...
    public function authorize(): bool
    {
        ...
        return $this->user()?->can('update', $this->route(strtolower('RedisHealth'))) ?? false;
    }
    ...
    {
        $id = $this->route(strtolower('RedisHealth'))?->id ?? $this->route('id');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique(strtolower('RedisHealths'))->ignore($id)],
    ...
    {
        logger()->warning('Update validation failed for UpdateRedisHealthRequest', [
