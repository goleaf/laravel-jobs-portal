                            ...
                            <tbody>
                                @forelse($admins as $admin)
                                    <tr>
                                        ...
                                        <td>{{ $admin->email }}</td>
                                        <td>
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded -full text-xs font-medium bg-{{ $admin->is_active ?"success' : 'danger' }}">
                                                {{ $admin->is_active ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td>{{ $admin->created_at->format('M d, Y') }}</td>
                                        <td>
                                            <div class="inline-flex items-center px-4 py-2 rounded font-medium transition-colors group-sm" role="group">
                                                <a href="{{ route('admin.', $admin) }}" class="border border-gray-300 bg-transparent">
                                                ...
                                                @if($admin->id !== auth()->id())
                                                    <form action="{{ route('admin.', $admin) }}" method="POST" class="inline">
                                                        @csrf
                                                        @method('DELETE')
