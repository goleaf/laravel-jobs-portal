...
/**
 * Universal API Request for updating Job
...
class UpdateJobApiRequest extends FormRequest
{
    ...
    {
        $canUpdate = $this->user()?->tokenCan(strtolower('Job') . ':update') ?? false;
        $resource = $this->route(strtolower('Job'));
    ...
    {
        $id = $this->route(strtolower('Job'))?->id ?? $this->route('id');

        return [
            'name' => ['sometimes', 'string', 'max:255', Rule::unique(strtolower('Jobs'))->ignore($id)],
