    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="shadow rounded bg-white -lg -xl max-w-lg w-full">
            <div class="border border border border-gray-300 -gray-300 px-6 py-4 -b -gray-200">
                <h3 class="fixed inset-0 z-50 overflow-y-auto -title">{{ __('messages.plan.new_subscription_plan') }}</h3>
            ...
            <div class="px-6 py-4">
                ...
                <div class="mb-5">
                    ...
                    {{ Form::text('name', null, ['id'=>'marital_status','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm','text-red-500','placeholder' => __('messages.inquiry.name')]) }}
                </div>
                <div class="mb-5">
                    {{ Form::label('allowed_jobs', __('messages.plan.allowed_jobs').(':'), ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
                    <span class="required"></span>
                    {{ Form::text('allowed_jobs', null, ['id'=>'allowedJobs','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm allowed-jobs','text-red-500', 'min' => '1', 'max' => '99999', 'placeholder' => __('messages.plan.allowed_jobs'), 'onkeyup' => 'if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,"")']) }}
                </div>
                <div class="mb-5">
                    {{ Form::label('plan_currency', __('messages.plan.currency').(':'), ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
                    <span class="required"></span>
                    {{ Form::select('salary_currency_id',[], null, ['id'=>'currency','text-red-500','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm-solid select2Selector','placeholder' => __('messages.company.select_currency')]) !!}
                </div>
                <div class="mb-5">
                    {{ Form::label('amount', __('messages.plan.amount').(':'), ['class' => 'block text-sm font-medium text-gray-700 mb-1']) }}
                    <span class="required"></span>
                    {{ Form::text('amount', null, ['id'=>'amount','class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm amount','text-red-500', 'min' => '1', 'max' => '99999', 'placeholder' => __('messages.plan.amount')]) }}
                </div>
            </div>
