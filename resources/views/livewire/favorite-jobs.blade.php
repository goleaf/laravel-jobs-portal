                    ...
                    @if(count($favouriteJobs) > 0 || $searchByJob != '' || $filterFavouriteJobs != '')
                        <div class="flex flex-wrap mb-2 justify-end">
                            <div class="flex-1 md-3">
                                {{ Form::select('favourite-jobs', $jobStatus, null, ['class' => 'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm ','id'=>'favouriteJobsId','placeholder' => __('messages.common.all'), 'wire:model' =>"filterFavouriteJobs"]) }}
                            </div>
                            <div class="flex-1 md-3">
