...
use App\Http\Requests\Auth\UpdateConfirmPasswordRequest;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

/**
 * Universal Enhanced Validation Tests for UpdateConfirmPasswordRequest.
 *
 * @group validation
 * @group requests
 *
 ...
 * @coversNothing
 */
class UpdateConfirmPasswordRequestTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $request = new UpdateConfirmPasswordRequest;
    ...
    /** @test */
    public function test_confirm_password_validation()
    {
        $request = new UpdateConfirmPasswordRequest;
        $rules = $request->rules();

        $this->assertArrayHasKey('ConfirmPassword', $rules);

        // Test field-specific validation rules
        $fieldRules = $rules['ConfirmPassword'];
    ...
    /** @test */
    public function test_confirm_passwords_validation()
    {
        $request = new UpdateConfirmPasswordRequest;
        $rules = $request->rules();

        $this->assertArrayHasKey('ConfirmPasswords', $rules);

        // Test field-specific validation rules
        $fieldRules = $rules['ConfirmPasswords'];
        $this->assertNotEmpty($fieldRules);
    }
    ...
    {
        $validData = [
            'ConfirmPassword' => 'Test Value',
            'id' => 1,
            'name' => 'Test Value',
            'required' => 'Test Value',
            'string' => 'Test Value',
            'ConfirmPasswords' => 'Test Value',
