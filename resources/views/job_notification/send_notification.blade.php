<div class="flex-wrap flex mainJobNotification">
    <div class="mb-4 flex-1 px-4 -xl-3 md:w-3/12 flex-1 sm-12 select-candidate-width">
    ...
    <div class="mb-4 flex-1 px-4 -xl-9 md:w-9/12 flex-1 sm-12">
        <ul class="ml-5 list-none job-notification-ul">
            @forelse($jobs as $key => $job)
                <li class="shadow rounded p-4 mt-4 media notification">
                    <div class="pt-1 mb-4 mb-0 md:w-4/12 flex-1 sm-12">
                        <label class="flex items-center form-switch form-switch-sm">
                            <input type="checkbox" name="job_id[]"
                                   ...
                                   value="{{ $job->id }}">
                            <a href="{{ route('admin.',$job->id) }}" target="_blank"
                               class="mb-1 media-title notification__title flex items-center label ms-5 text-decoration-none">{{ \Illuminate\Support\Str::limit($job->job_title,65) }}</a>
                        </label>
