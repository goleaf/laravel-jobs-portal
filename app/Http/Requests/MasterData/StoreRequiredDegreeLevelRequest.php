...
/**
 * Universal Form Request for storing RequiredDegreeLevel
 * Implements Laravel 12 best practices with Universal MCP patterns
 */
class StoreRequiredDegreeLevelRequest extends FormRequest
{
    /**
    ...
    {
        return $this->user()?->can('create', requireddegreelevel::class) ?? true;
    }
    ...
    {
        return [
            'name' => ['required', 'string', 'max:255', 'unique:requireddegreelevels,name'],
    ...
    {
        logger()->info('Store validation failed for StoreRequiredDegreeLevelRequest', [
