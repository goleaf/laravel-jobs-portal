...
/**
 * Context7 API Request for updating Skill
...
class UpdateSkillApiRequest extends FormRequest
{
    ...
    {
        $id = $this->route(strtolower('Skill'))?->id ?? $this->route('id');

        return [
            'name' => ['sometimes', 'string', 'max:255', Rule::unique(strtolower('Skills'))->ignore($id)],
