...
/**
 * 🚀 UNIVERSAL REQUEST & TEST FILE GENERATOR
 * 
 * Generates missing request and test files using Universal MCP patterns
 * Based on Laravel 12 best practices from Universal documentation
 */

echo "\n🚀 UNIVERSAL REQUEST & TEST FILE GENERATOR\n";
echo "=" . str_repeat("=", 50) . "\n\n";

class UniversalRequestTestGenerator
{
    ...
    {
        echo "📝 **GENERATING UNIVERSAL REQUEST FILES**\n";
        echo "-" . str_repeat("-", 40) . "\n\n";
        
        $this->generateRequestFiles();
        
        echo "\n🧪 **GENERATING UNIVERSAL TEST FILES**\n";
        echo "-" . str_repeat("-", 35) . "\n\n";
        ...
        echo "✅ Following Universal MCP best practices\n";
    ...
    private function generateApiStoreRequest($controller)
    {
        ...
        $requestDir = "app/Http/Requests/Api/Universal/";
        if (!is_dir($requestDir)) {
            mkdir($requestDir, 0755, true);
        }
...
/**
 * Universal Form Request for updating {$entity}
 * Implements Laravel 12 best practices with Universal MCP patterns
 */
class {$requestName} extends FormRequest
{
...
/**
 * Universal API Request for storing {$entity}
...
/**
 * Universal API Request for updating {$entity}
 * Implements Laravel 12 API best practices with Universal MCP patterns
 */
class {$requestName} extends FormRequest
{
...
";
    ...
    private function generateApiControllerTest($controller)
    {
        ...
        $testDir = "tests/Feature/Api/Universal/";
        if (!is_dir($testDir)) {
            mkdir($testDir, 0755, true);
        }
...
/**
 * Universal API Test for {$controller}
...
// Run the generator
try {
    $generator = new UniversalRequestTestGenerator();
    $generator->generateAll();
    
    echo "\n" . str_repeat("=", 70) . "\n";
    echo "🚀 UNIVERSAL REQUEST & TEST FILE GENERATION COMPLETE!\n";
    echo str_repeat("=", 70) . "\n";
